    ...
    <div class="bg-white p-4 rounded shadow-sm">
      ...
      <div class="table-responsive">
        <table class="table table-bordered text-center align-middle">
          ...
          <tbody id="pkAdminTableBody">
            <!-- Default message -->
            <tr id="defaultMessage" style="display: block;">
              <td colspan="5" class="text-center text-muted py-4">
                <i class="fas fa-info-circle mb-2"></i><br>
                Silakan pilih Unit Kerja untuk menampilkan data PK Administrator
              </td>
            </tr>
            
            <!-- Data will be populated here by JavaScript -->
            <?php if (!empty($pkAdminData) && is_array($pkAdminData)): ?>
              <?php $globalNo = 1; ?>
              <?php foreach ($pkAdminData as $pk): ?>
...
function showNoDataMessage() {
    const tbody = document.getElementById('pkAdminTableBody');
    const noDataRow = document.createElement('tr');
    noDataRow.className = 'no-data-message';
    noDataRow.innerHTML = `
        <td colspan="5" class="text-center text-muted py-4">
            <i class="fas fa-search mb-2"></i><br>
            Tidak ada data PK Administrator untuk filter yang dipilih
        </td>
    `;
