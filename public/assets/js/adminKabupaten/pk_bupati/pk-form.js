document.addEventListener('DOMContentLoaded', function() {
    ...
    function updateFormNames() {
        ...
        document.querySelectorAll('.program-container .program-item').forEach((program, index) => {
            ...
            const anggaranInput = program.querySelector('input[name*="anggaran"], .anggaran-display');
    ...
    document.querySelector('.add-program').addEventListener('click', function() {
        ...
        // Ambil select pertama dan buat versi clean tanpa selected option
        const firstSelect = document.querySelector('.program-select');
        let cleanOptions = '';
        
        if (firstSelect) {
            Array.from(firstSelect.options).forEach(option => {
                const selected = option.value === '' ? ' selected' : ''; // Hanya default option yang selected
                cleanOptions += `<option value="${option.value}"${selected} data-anggaran="${option.getAttribute('data-anggaran') || ''}">${option.textContent}</option>`;
            });
        }
            ...
            <div class="mb-3">
                <div class="row">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="text" class="form-control mb-3 border-secondary anggaran-display" placeholder="Anggaran" readonly>
                    </div>
                </div>
            </div>
        `;
    ...
    document.addEventListener('change', function(e) {
        if (e.target.classList.contains('program-select')) {
            ...
            const anggaranInput = e.target.closest('.row').querySelector('.anggaran-display');
