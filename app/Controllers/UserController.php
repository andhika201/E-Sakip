...
class UserController extends BaseController
{
    ...
    private function buildCascadingKabFirstShow($rows)
    {
        ...
        foreach ($rows as $index => $r) {
            ...
            $key = $r['indikator_id'] . '-' . $r['nama_opd'];
            if (!isset($shown['opd'][$key])) $shown['opd'][$key] = $index;
        }
        return $shown;
    }
    ...
    public function cascading_opd_cetak()
    {
        ...
        if (!$periode || !$opd_id) return redirect()->back();
    ...
    {
        $meta = [
            'tujuan' => [], 'sasaran' => [], 'tujuan_renstra' => [],
            'sasaran_renstra' => [], 'indikator' => [], 'es3' => [],
            'es3_indikator' => [], 'es4' => [],
        ];
        foreach ($rows as $r) {
            $meta['tujuan'][$r['tujuan_id']] = ($meta['tujuan'][$r['tujuan_id']] ?? 0) + 1;
            ...
            $meta['indikator'][$r['indikator_id']] = ($meta['indikator'][$r['indikator_id']] ?? 0) + 1;
            if ($r['es3_id']) $meta['es3'][$r['es3_id']] = ($meta['es3'][$r['es3_id']] ?? 0) + 1;
            $key = $r['es3_id'] . '_' . ($r['es3_indikator_id'] ?? null);
            $meta['es3_indikator'][$key] = ($meta['es3_indikator'][$key] ?? 0) + 1;
            if ($r['es4_id']) $meta['es4'][$r['es4_id']] = ($meta['es4'][$r['es4_id']] ?? 0) + 1;
        }
        return $meta;
    }
    ...
    private function buildOpdTree($rows)
    {
        ...
        foreach ($rows as $r) {
            ...
            if (empty($r['renstra_sasaran_id']) && empty($r['renstra_sasaran'])) continue;
            ...
            if ($risId) $tree[$tId]['sasarans'][$sId]['tujuan_renstras'][$rtId]['es2s'][$rsId]['indikators'][$risId] = $r['indikator_sasaran'];
            $es3Id = $r['es3_id'];
            if ($es3Id) {
                ...
                if ($es3IndId) $tree[$tId]['sasarans'][$sId]['tujuan_renstras'][$rtId]['es2s'][$rsId]['es3s'][$es3Id]['indikators'][$es3IndId] = $r['es3_indikator'];
            }
        }
        return $tree;
    }
