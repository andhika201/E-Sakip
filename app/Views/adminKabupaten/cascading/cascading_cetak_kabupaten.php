...
<head>
    ...
    <style>
        ...
        .pdf-visi { font-weight: bold; font-size: 10px; margin: 4px 0 8px; color: #15311f; }
        table.pdf-table td, table.pdf-table th { font-size: 8px; }
    </style>
</head>
...
<body>
    ...
    <?= $this->include('templates/pdf_kop', [
        ...
        'subjudul' => 'Visi · Misi · Tujuan · Sasaran · Indikator · Program · Perangkat Daerah · Periode ' . esc($periodeTxt),
    ...
    <table class="pdf-table">
        <thead>
            <tr>
                <th rowspan="2" style="min-width:110px;">Misi</th>
                <th rowspan="2" style="min-width:110px;">Tujuan</th>
                <th rowspan="2" style="min-width:110px;">Sasaran</th>
                <th rowspan="2" style="min-width:110px;">Indikator</th>
                <th rowspan="2" style="min-width:150px;">Program</th>
                <th rowspan="2" style="min-width:120px;">Perangkat Daerah Penanggung Jawab</th>
                                ...
                                <?php foreach ($ind['opd'] as $opd): $oSpan = $opdRows($opd); $oPr = false;
                                    ...
                                    <?php $pNo = 0; foreach ($progs as $prog): $pNo++; ?>
                                        <tr>
                                            ...
                                            <td class="text-start"><?php if (!$mPr): ?><?= $miNo ?>. <?= nl2br(esc($m['misi'])) ?><?php $mPr = true; endif; ?></td>
                                            <td class="text-start"><?php if (!$tPr): ?><?= $tuNo ?>. <?= nl2br(esc($t['nama'])) ?><?php $tPr = true; endif; ?></td>
                                            <td class="text-start"><?php if (!$sPr): ?><?= $saNo ?>. <?= nl2br(esc($s['nama'])) ?><?php $sPr = true; endif; ?></td>
