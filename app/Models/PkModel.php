...
class PkModel extends Model
{
    ...
    public function getAllProgramPkGrouped()
    {
        $results = $this->db->table('pk_program pp')
            ->select('pp.pk_id, pr.program_kegiatan, pr.anggaran')
            ->join('program_pk pr', 'pr.id = pp.program_id')
            ->orderBy('pp.pk_id', 'ASC')
            ->get()
            ->getResultArray();

        // Grouping data berdasarkan pk_id
        $grouped = [];

        foreach ($results as $row) {
            $pkId = $row['pk_id'];

            if (!isset($grouped[$pkId])) {
                $grouped[$pkId] = [];
            }
