...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const templateIndikator = document.querySelector('.indikator-item')?.cloneNode(true);
  const templateProgramItem = document.querySelector('.program-item')?.cloneNode(true);
  const templateKegiatanItem = document.querySelector('.kegiatan-item')?.cloneNode(true);
  ...
  function updateFormNames() {
    ...
    $$('.sasaran-item', sasaranContainer).forEach((sasaranItem, si) => {
      ...
      $$('.indikator-item', sasaranItem).forEach((indikatorItem, ii) => {
        ...
        const selectJenis = indikatorItem.querySelector('select[name*="[jenis_indikator]"]');

        if (inputInd) inputInd.name = `sasaran_pk[${si}][indikator][${ii}][indikator]`;
        if (inputTarget) inputTarget.name = `sasaran_pk[${si}][indikator][${ii}][target]`;
        if (selectSatuan) selectSatuan.name = `sasaran_pk[${si}][indikator][${ii}][id_satuan]`;
  ...
  document.body.addEventListener('click', (ev) => {
    ...
    if (target.closest('.add-kegiatan')) {
      ...
      const wrapper = target.closest('.program-item') || target.closest('.sasaran-item') || target.closest('.indikator-item');
      let kegiatanContainer = wrapper ? wrapper.querySelector('.kegiatan-container') : null;
      if (!kegiatanContainer) {
        ...
        kegiatanContainer = indik ? indik.querySelector('.kegiatan-container') : null;
      }
      if (!kegiatanContainer) return;

      const proto = kegiatanContainer.querySelector('.kegiatan-item') ? kegiatanContainer.querySelector('.kegiatan-item').cloneNode(true) : (templateKegiatanItem ? templateKegiatanItem.cloneNode(true) : null);
      if (!proto) return;
      clearFormControls(proto);
    ...
    if (target.closest('.add-subkeg')) {
      ...
      const subContainer = kegiatanItem.querySelector('.subkeg-container') || kegiatanItem.querySelector('.subkeg-container') || kegiatanItem;
      if (!subContainer) return;
      const proto = subContainer.querySelector('.subkeg-item') ? subContainer.querySelector('.subkeg-item').cloneNode(true) : (templateSubkegItem ? templateSubkegItem.cloneNode(true) : null);
      if (!proto) return;
      clearFormControls(proto);
  ...
  document.body.addEventListener('change', (ev) => {
    ...
    if (tgt.matches('select.program-select, select[name*="[program_id]"]')) {
      ...
      if (!programItem) return;
      const anggaranField = programItem.querySelector('input[name*="[anggaran]"]');
