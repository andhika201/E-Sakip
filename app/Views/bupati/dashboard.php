              ...
              <button type="button" class="kpi" data-drawer="f_capaian">
                ...
                <div>
                  ...
                  <?php else: ?>
                    ...
                    <div class="kpi-sub mt-2">
                      <?php if ($fCapaian['has_monev_average'] ?? false): ?>
                        <?= $sorot('fa-chart-line', '#3f6296', 'Rata-rata Realisasi KMonev') ?>
                      <?php endif; ?>
                      <?= $sorot('fa-list-ol', '#3f6296', (int) $fCapaian['valid'] . ' dari ' . (int) $fCapaian['wajib'] . ' indikator valid') ?>
            ...
            <button type="button" class="kpi" data-drawer="pk_bupati">
              ...
              <div>
                <?php if (!$lkB['ada']): ?>
                  ...
                  <div class="kpi-sub mt-2">
                    <?= $sorot('fa-file-circle-question', '#8a968f', 'LAKIP Kabupaten tahun ' . (int) $lkB['tahun_lakip'] . ' belum memiliki indikator untuk dinilai') ?>
                  </div>
                <?php elseif ($lkB['can_compute']): ?>
                  <div class="kpi-num" style="color:<?= esc($lkB['status']['color_hex']) ?>"><?= esc(capaianFormatPersen($lkB['total'])) ?></div>
                  <div class="kpi-sub mt-2">
                    <?= $sorot('fa-file-lines', '#3f6296', 'LAKIP Kabupaten ' . (int) $lkB['tahun_lakip'] . ' &middot; ' . esc($lkB['sumber_label'] ?? '')) ?>
                    <?= $sorot(
                          $lkB['final'] ? 'fa-circle-check' : 'fa-hourglass-half',
                          $lkB['final'] ? '#0a8f50' : '#e07b39',
                          esc($lkB['label'])
                        ) ?>
                    <?= $sorot('fa-list-ol', '#3f6296', (int) $lkB['valid'] . ' dari ' . (int) $lkB['wajib'] . ' indikator terhitung'
                          . ($lkB['kritis'] > 0 ? ' &middot; ' . (int) $lkB['kritis'] . ' kritis' : '')) ?>
                    <?= $sorot('fa-flag', $lkB['status']['color_hex'], esc($lkB['status']['name'])) ?>
                  </div>
                <?php else: ?>
                  <div class="kpi-num sm text-muted">Belum dapat dihitung</div>
                  <div class="kpi-sub mt-2">
                    <?= $sorot('fa-file-lines', '#3f6296', 'LAKIP Kabupaten ' . (int) $lkB['tahun_lakip'] . ' &middot; ' . esc($lkB['sumber_label'] ?? '')) ?>
                    <?= $sorot('fa-list-ol', '#3f6296', (int) $lkB['valid'] . ' dari ' . (int) $lkB['wajib'] . ' indikator terhitung') ?>
                    <?= $sorot('fa-triangle-exclamation', '#d9a520', (int) $lkB['belum_valid'] . ' indikator perlu dilengkapi'
                          . ($lkB['belum_input'] > 0 ? ' (' . (int) $lkB['belum_input'] . ' belum ada realisasi)' : '')) ?>
                  </div>
                <?php endif; ?>
              </div>
      ...
      function kartuOpd(o) {
        ...
        return '<div class="ind-card">' +
          '<div class="d-flex justify-content-between gap-2 align-items-start">' +
            ...
            '<div class="ind-pct">' + (o.can_compute ? pct(o.percentage) : (o.has_monev_average ? pct(o.monev_average) : '')) + '</div>' +
          '</div>' +
          '<div class="ind-meta">' +
          ...
          '<div class="d-flex flex-wrap gap-2 mt-2 align-items-center">' + badge(o.status) +
            '<span class="badge-soft" style="background:' + updBg + ';color:' + updColor + ';"><i class="fas fa-clock-rotate-left"></i>' + esc(o.update.label) + '</span>' +
      ...
      function kartuIndikatorBupati(i) {
        ...
        return '<div class="ind-card" data-pk="' + i.indikator_id + '" role="button">' +
          ...
          '<div class="d-flex flex-wrap gap-2 mt-2">' + badge(i.status) +
            '<span class="badge-soft" style="background:' + validBg + ';color:' + validColor + ';"><i class="fas ' + (i.is_valid ? 'fa-circle-check' : 'fa-calculator') + '"></i>' +
              (i.is_valid ? 'Valid' : 'Belum valid') + '</span>' +
            // Lencana status verifikasi hanya bila mekanismenya ada (lihat verificationInfo()).
            (i.verification.available ? '<span class="badge-soft" style="background:#fdf0e6;color:#e07b39;"><i class="fas fa-hourglass-half"></i>' + esc(i.verification.label) + '</span>' : '') +
          '</div>' +
        ...
        return '<div class="ind-card">' +
          ...
          '<div class="d-flex flex-wrap gap-2 mt-2">' + badge(i.status) +
            '<span class="badge-soft" style="background:#f1f3f2;color:#6b7a70;"><i class="fas ' + (i.is_valid ? 'fa-circle-check' : 'fa-calculator') + '"></i>' +
              (i.is_valid ? 'Valid' : 'Belum valid') + '</span>' +
            (i.verification.available ? '<span class="badge-soft" style="background:#fdf0e6;color:#e07b39;"><i class="fas fa-hourglass-half"></i>' + esc(i.verification.label) + '</span>' : '') + '</div>' +
          (i.reason ? '<div class="ins-why mt-2"><i class="fas fa-circle-info me-1"></i>' + esc(i.reason) + '</div>' : '') +
          ...
          body.innerHTML =
            '<div class="drawer-section">' +
              ...
              '<div class="d-flex flex-wrap gap-2 mb-3">' + badge(d.status) +
                ...
                (d.verification.available ? '<span class="badge-soft" style="background:#fdf0e6;color:#e07b39;"><i class="fas fa-hourglass-half"></i>' + esc(d.verification.label) + '</span>' : '') + '</div>' +
